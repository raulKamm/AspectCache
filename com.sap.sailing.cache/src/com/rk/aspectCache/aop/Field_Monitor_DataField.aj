...
import com.sap.sailing.cache.common.DataField;
...
public aspect Field_Monitor_DataField extends Abstract_Field_Monitor{
	...
	pointcut dataFieldGet(Object obj): get(@DataField !final (!(Object+[]) && !(int[])) && !(byte[]) && !(short[]) && !(long[]) && !(float[]) && !(double[]) && !(boolean[]) && !(char[]) *) && cflow(cachedMethod()) && target(obj) && !within(com.sap.sailing.cache..*);
	// picks all set of non-final, non-static, non-array type fields
	pointcut dataFieldSet(Object obj): set(@DataField !final (!(Object+[]) && !(int[])) && !(byte[]) && !(short[]) && !(long[]) && !(float[]) && !(double[]) && !(boolean[]) && !(char[]) *) && target(obj) && !within(com.sap.sailing.cache..*);
	// picks all references to static, non-final, non-array type fields happening in the control flow of cachedMethod()
	pointcut dataFieldGetStatic(): get(@DataField static !final (!(Object+[]) && !(int[])) && !(byte[]) && !(short[]) && !(long[]) && !(float[]) && !(double[]) && !(boolean[]) && !(char[]) *) && cflow(cachedMethod()) && !within(com.sap.sailing.cache..*);
	// picks all set of static, non-final, non-array type fields
	pointcut dataFieldSetStatic(): set(@DataField static !final (!(Object+[]) && !(int[])) && !(byte[]) && !(short[]) && !(long[]) && !(float[]) && !(double[]) && !(boolean[]) && !(char[]) *) && !within(com.sap.sailing.cache..*);
